...
| **同源 TTS 发音** | 单词有道（缺条目自动回退百度/Google）/ 例句百度(主)+Google(备) 整句自然朗读，手机端不依赖任何外部域名 |
...
| 输入 | 引擎路由 | 说明 |
|---|---|---|
| **单词**（无空格且 ≤40 字符）| **有道 `dictvoice`（主）** → 百度 → Google（兜底）| 英式无音频自动回退美式（双口音）；有道词库缺条目（如连字符词 `time-box`）会返回 500，此时自动回退百度/Google，**保证任何词都能出声**；响应头 `X-TTS-Source: youdao-proxy / baidu-word / google-word` |
