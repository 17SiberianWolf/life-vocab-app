...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* ===== 拼写游戏 (Spell) ===== */
  .spell-stage { text-align: center; padding: 18px 12px 6px; }
  .spell-hint-label { font-size: 12px; color: var(--text-muted); margin-bottom: 10px; }
  .spell-zh { font-size: 26px; font-weight: 700; line-height: 1.45; margin-bottom: 14px; color: var(--text); }
  .spell-audio { display: flex; gap: 10px; justify-content: center; margin-bottom: 8px; }
  .spell-reveal { min-height: 24px; font-size: 14px; color: var(--text-soft); margin-top: 6px; }
  .spell-reveal.mask { font-family: ui-monospace, SFMono-Regular, Menlo, monospace; letter-spacing: 2px; font-size: 18px; color: var(--accent-strong); }
  .spell-warn { min-height: 18px; font-size: 13px; color: var(--warn); text-align: center; margin: 6px 0 0; font-weight: 600; }
  .spell-input-row { display: flex; gap: 8px; flex-wrap: wrap; align-items: center; margin: 14px 0; }
  .spell-input {
    flex: 1; min-width: 180px;
    border: 2px solid var(--border); border-radius: var(--radius);
    padding: 12px 14px; font-size: 20px; letter-spacing: 1px;
  ...
  @media (max-width: 600px) {
    ...
    /* 拼写: 输入框字号 ≥16px 防 iOS 聚焦自动缩放; 触控目标 ≥44px */
...
<header class="top">
  ...
  <button class="navbtn" data-go="spell">✍ 拼写</button>
...
<main>
  ...
  <section id="view-topics" class="view active">
    ...
    <p style="color: var(--text-soft); margin-top:0; margin-bottom:18px; font-size:14px;">
      <span id="topicsSummary">点击主题进入 7 种玩法。</span>
    </p>
  ...
  <!-- 6 个游戏视图 (Match / Listen / Memory / Gravity / Spell / Browse) -->
  ...
  <section id="view-match" class="view">
    <div class="match-head">
      ...
      <div class="game-stats">
        ...
        <button class="navbtn" id="matchRestart">↻ 重开</button>
  ...
  <section id="view-spell" class="view">
    <div class="view-title-row">
      <button class="navbtn" data-go="topics">← 主题</button>
    ...
    <div class="spell-stage">
      ...
      <div class="spell-audio">
        ...
        <button class="pill" id="spellPlaySlow">🐢 慢速</button>
      </div>
      <div class="spell-reveal" id="spellReveal"></div>
    </div>
    <div class="spell-warn" id="spellWarn"></div>
    <div class="spell-input-row">
      <input id="spellInput" class="spell-input" type="text" inputmode="text"
             autocomplete="off" autocorrect="off" autocapitalize="none"
             spellcheck="false" enterkeyhint="done" placeholder="输入英文拼写…" />
      <button class="navbtn primary" id="spellSubmit">提交</button>
      <button class="pill" id="spellHint">💡 提示</button>
      <button class="pill" id="spellSkip">跳过</button>
    </div>
    <div class="game-stats">
      <div>已答:<b id="spellAnswered">0</b></div>
      <div>正确:<b id="spellCorrect">0</b></div>
      <div>连击:<b id="spellStreak">0</b></div>
      <div>用时:<b id="spellTimer">0.0s</b></div>
      <button class="navbtn" id="spellRestart">换一批</button>
    </div>
  </section>
...
(function () {
  ...
  function stopAllGames() {
    stopMatchGame(); stopMemoryGame(); stopListenGame(); stopGravityGame(); stopSpellGame();
    ...
    _onlineEpoch++;   // 使任何待执行的 advance 回调失效
    _onlineQueue = [];
  ...
  function show(name, topicId) {
    ...
    if (name === 'spell') renderSpell(topicId);
  ...
  function renderTopics() {
    ...
    if (sumEl) sumEl.textContent = '共 ' + STATE.topics.length + ' 个主题、' + STATE.cards.length + ' 张卡，点击主题进入 7 种玩法。';
    STATE.topics
      .slice()
      ...
      .forEach(t => {
        ...
        card.innerHTML = `
          ...
          <div class="games">
            ...
            <button class="game-btn" data-action="spell" data-topic="${t.topic_id}">✍ 拼写</button>
  ...
  function _playOnlineNext(slow, silentFail, epoch) {
    if (epoch !== _onlineEpoch) return;          // 已有新请求, 放弃本帧
    let item = null;
    while (_onlineQueue.length) {
      const t = _onlineQueue.shift();
      if (t && t.trim()) { item = t.trim(); break; }
    }
    if (!item) { _onlinePlaying = false; _onlineAudio = null; return; }
    _onlinePlaying = true;
    const type = (STATE.settings.locale === 'en-US') ? 2 : 1;   // 1=英式 2=美式
    const a = new Audio('/tts?audio=' + encodeURIComponent(item) + '&type=' + type);
    a.playbackRate = slow ? 0.85 : (STATE.settings.rate || 1.0);
    _onlineAudio = a;
    let done = false;
    const advance = () => {
      ...
      if (_onlineAudio === a) _onlineAudio = null;
      _onlinePlaying = false;
      _playOnlineNext(slow, silentFail, epoch);
    };
    a.addEventListener('ended', advance);
    a.addEventListener('error', () => {
      // 该词在线发音失败: 单 token 时提示并尝试本地语音兜底
      if (_onlineSingle && !silentFail) {
        ttsHint('在线发音加载失败, 尝试本地语音', 'warn');
        try { if (TTS.ok) { loadVoices(); unlockTTS(); speakWeb(item, slow); } } catch (e) {}
      }
      advance();
    });
    a.addEventListener('stalled', advance);
    const p = a.play();
    if (p && p.catch) p.catch(advance);
    // 兜底: 单 token 卡死(既不 ended 也不 error)时强制推进, 避免整句卡住
  ...
  function renderMatch(topicId) {
    ...
    // C: 把吸顶偏移量设为全局顶栏实际高度, 使 match-head 精确贴在其下方(移动端)
    const topbar = document.querySelector('header.top');
    if (topbar) document.documentElement.style.setProperty('--lv-topbar', topbar.offsetHeight + 'px');
    startRound();
  }
  ...
  // ============================================================
  // Spell 拼写 (看中文 + 听音 -> 打字拼写)
  // ============================================================
  // 判定归一: 仅统一"同义字符"(弯引号/连字符变体) + 忽略大小写。
  // 空格(含首尾/连续)与非法特殊字符一律**不做任何处理**, 严格比对。
  ...
  // 逐字符比对, 返回首个不匹配位置(0 基); 输入是正确前缀返回 -1; 多打的字符也算错
  function firstSpellMismatch(typed, expected) {
    const norm = (x) => String(x == null ? '' : x)
      .replace(/[\u2018\u2019`\u00b4]/g, "'")
      .replace(/[\u2013\u2014\u2212]/g, '-');
    const a = norm(typed), b = norm(expected);
    const n = Math.min(a.length, b.length);
    for (let i = 0; i < n; i++) {
      if (a[i].toLowerCase() !== b[i].toLowerCase()) return i;
    }
    return a.length > b.length ? b.length : -1;
  }
  // 提示遮罩: 保留空格/连字符/撇号, 揭示前 revealed 个字母(数字), 其余为 _
  function hintText(en, revealed) {
    let letters = 0, out = '';
    for (const ch of String(en == null ? '' : en)) {
      if (/[a-z0-9]/i.test(ch)) {
        out += (letters < revealed) ? ch : '_';
        letters++;
      } else if (ch === ' ') {
        out += '  ';
      } else {
  ...
  // 拼写题源: 仅取"纯英文(含空格/撇号/连字符)"的卡片,
  // 排除含括号/斜杠/句点等占位短语(如 "pear (Asian)" / "A / B"), 否则严格判定下无法作答。
  const SPELL_EN_OK = /^[A-Za-z][A-Za-z'\- ]*$/;
  function spellLetterCount(en) { return (String(en || '').match(/[a-z0-9]/gi) || []).length; }
  ...
  function renderSpell(topicId) {
    ...
    document.getElementById('spellTitle').innerHTML = topic
      ...
      : `全部词库 <span style="color:var(--text-muted);font-weight:400;font-size:13px;">Spell</span>`;
    startSpellRound();
  }
  function startSpellRound() {
    // 清掉旧回合的计时器, 避免"换一批"时定时器泄漏
    if (STATE.spellRound && STATE.spellRound.timer) clearInterval(STATE.spellRound.timer);
    const pool = STATE.cards.filter(c =>
      (!STATE.currentTopic || c.topic === STATE.currentTopic) && SPELL_EN_OK.test(c.en || '')
    );
    if (!pool.length) {
      STATE.spellRound = null;
      alert('该主题暂无可出题的卡片');
    ...
    const r = {
      queue, cursor: 0, current: null,
      correct: 0, answered: 0, streak: 0, maxStreak: 0,
      attempts: 0, wrongRecorded: false, hintLevel: 0, errAt: -1,
      startTime: performance.now(), timer: null
    };
    STATE.spellRound = r;
    r.timer = setInterval(() => {
      if (STATE.spellRound !== r) return;
      const el = document.getElementById('spellTimer');
      if (el) el.textContent = ((performance.now() - r.startTime) / 1000).toFixed(1) + 's';
    }, 200);
    nextSpell();
  }
  function nextSpell() {
    const r = STATE.spellRound;
    if (!r) return;
    if (r.cursor >= r.queue.length) { endSpellRound(); return; }
    const cur = r.queue[r.cursor];
    r.current = cur;
    r.attempts = 0; r.wrongRecorded = false; r.hintLevel = 0; r.errAt = -1;
    const input = document.getElementById('spellInput');
    input.value = '';
    input.disabled = false;
    input.classList.remove('wrong', 'correct');
    document.getElementById('spellZh').textContent = cur.zh || '';
    const rev = document.getElementById('spellReveal');
    rev.textContent = '';
    rev.classList.remove('mask');
    document.getElementById('spellWarn').textContent = '';
    ...
    if (!IS_MOBILE) setTimeout(() => { if (STATE.spellRound === r && !input.disabled) input.focus(); }, 60);
  }
  function submitSpell() {
    ...
    if (!val) return;                      // 空值忽略(空格也算有值 -> 严格判错)
    const warn = document.getElementById('spellWarn');
    const rev = document.getElementById('spellReveal');
    if (normalizeSpelling(val) === normalizeSpelling(r.current.en)) {
      ...
      document.getElementById('spellStreak').textContent = r.streak;
      rev.classList.remove('mask');
      rev.textContent = `✓ ${r.current.en} = ${r.current.zh}`;
      input.disabled = true;
      ...
      else if (at >= r.current.en.length) warn.textContent = `⚠ 多输了字符（应为 ${r.current.en.length} 个字符）`;
      else warn.textContent = `⚠ 第 ${at + 1} 个字符不正确`;
      document.getElementById('spellStreak').textContent = r.streak;
      if (r.attempts >= 3) {
        // 连错 3 次: 揭示答案并按错进入下一题
        r.answered++;
        rev.classList.remove('mask');
        rev.textContent = `✗ 正确答案：${r.current.en} = ${r.current.zh}`;
        warn.textContent = '✗ 已连错 3 次，看看答案';
        document.getElementById('spellAnswered').textContent = r.answered;
        input.disabled = true;
        setTimeout(() => {
          if (STATE.spellRound !== r) return;
  ...
  function showHint() {
    const r = STATE.spellRound;
    if (!r || !r.current) return;
    const en = r.current.en;
    const letterCount = spellLetterCount(en);
    const maxReveal = Math.max(0, letterCount - 1);
    r.hintLevel = Math.min(r.hintLevel + 1, maxReveal);
    const rev = document.getElementById('spellReveal');
    rev.classList.add('mask');
    rev.textContent = hintText(en, r.hintLevel) + `   (${letterCount} 个字母)`;
  }
  function skipSpell() {
    const r = STATE.spellRound;
    if (!r || !r.current) return;
    const rev = document.getElementById('spellReveal');
    rev.classList.remove('mask');
    rev.textContent = `答案：${r.current.en} = ${r.current.zh}`;
    document.getElementById('spellWarn').textContent = '';
    const input = document.getElementById('spellInput');
    input.disabled = true;
    input.classList.remove('wrong', 'correct');
  ...
  function endSpellRound() {
    const r = STATE.spellRound;
    if (!r) return;
    if (r.timer) { clearInterval(r.timer); r.timer = null; }
    const secs = ((performance.now() - r.startTime) / 1000).toFixed(1);
    const acc = r.answered ? Math.round(r.correct / r.answered * 100) : 0;
    document.getElementById('spellZh').textContent = '本批完成 🎉';
    document.getElementById('spellWarn').textContent = '';
    const rev = document.getElementById('spellReveal');
    rev.classList.remove('mask');
    rev.textContent = `答对 ${r.correct}/${r.answered} · 正确率 ${acc}% · 最长连击 ${r.maxStreak} · 用时 ${secs}s`;
    document.getElementById('spellTimer').textContent = secs + 's';
    const input = document.getElementById('spellInput');
  ...
  document.getElementById('spellPlay').addEventListener('click', () => {
    const r = STATE.spellRound;
    if (r && r.current) speak(r.current.en, false);
  });
  document.getElementById('spellPlaySlow').addEventListener('click', () => {
    const r = STATE.spellRound;
    if (r && r.current) speak(r.current.en, true);
  });
  document.getElementById('spellSubmit').addEventListener('click', submitSpell);
  document.getElementById('spellHint').addEventListener('click', showHint);
  document.getElementById('spellSkip').addEventListener('click', skipSpell);
  document.getElementById('spellInput').addEventListener('input', () => {
    const r = STATE.spellRound;
    const input = document.getElementById('spellInput');
    if (!r || !r.current || input.disabled) return;
    const at = firstSpellMismatch(input.value, r.current.en);
    const warn = document.getElementById('spellWarn');
    if (at < 0) {                          // 正确前缀 / 完整正确 -> 清除警示
      input.classList.remove('wrong');
      warn.textContent = '';
      r.errAt = -1;
    } else {
      if (at !== r.errAt) {                // 出错位置变化才重抖, 避免持续闪烁
        input.classList.remove('wrong');
        void input.offsetWidth;            // 强制重排以重启动画
        input.classList.add('wrong');
        warn.textContent = (at >= r.current.en.length)
          ? `⚠ 多输了字符（应为 ${r.current.en.length} 个字符）`
          : `⚠ 第 ${at + 1} 个字符不正确`;
      }
      r.errAt = at;
    }
