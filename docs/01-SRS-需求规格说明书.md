...
| 需求点 | 描述 |
|---|---|
| FR-02.1 | 从当前主题随机抽取 **6 张卡**，生成 6 个英文 + 6 个中文（共 **12 张等大瓷砖**），打乱后展示；桌面 4 列 / 手机 3 列 |
| FR-02.2 | 玩家依次点击两张瓷砖，若属于同一张卡则**变绿并淡出清空**（Duolingo 式），否则两张闪红抖动并记为一次错误 |
| FR-02.3 | 实时显示用时（精度 0.1s）、错误次数、得分；顶部**绿色进度条**显示已配对组数 |
| FR-02.4 | 全部配对后结算：用时、错误数、得分，以**内嵌完成横幅**呈现（含「再来一轮」「返回主题」），不再用 alert 弹窗 |
| FR-02.5 | 提供"听词"按钮，按 700ms 间隔依次朗读本轮全部单词 |
| FR-02.6 | 点击英文瓷砖即时朗读该词；配对成功朗读该词并记录 SRS 结果 |
...
| AC-03 | 游戏功能 | 6 个游戏（碰碰乐/听音/连连看/消消乐/拼写/跟读）均可正常开局、作答、结算 |
