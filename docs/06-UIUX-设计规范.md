...
| 浏览 / 复习 / 错题本 | 单列或双列卡片流 |
| 碰碰乐 | 棋盘：6 英文 + 6 中文（12 张等大瓷砖）打乱铺开；桌面 4 列 / 手机 3 列；配对成功变绿淡出；顶部细进度条 |
| 连连看 | 12 枚翻牌（6 对） |
| 听音 | 顶部大播放区 + 4 个候选按钮 |
| 消消乐 | 顶部下落区 + 底部 4 候选 |

> **游戏视图统一居中窄栏**：`#view-match/-listen/-memory/-gravity/-spell/-record` 均限宽 `max-width: 640px` 并居中，
> 避免在桌面端继承 `main` 的 960px 被拉伸成"宽横条"。
